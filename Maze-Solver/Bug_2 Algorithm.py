...
def main_algorithm(grid, start, end):
    ...
    indx=0
    code=1
    line_path = line_algorithm(grid, start, end)   
    while main_path[-1] != end:
        for cnt in range(indx, len(line_path)-1):
            if line_path[cnt+1].color == BLUE:
                find_set(grid, line_path[cnt+1],code)
                start = line_path[cnt]
                path = go_round(grid, start, end, start, [1, start], line_path,code)
                path.pop(0)
                indx = line_path.index(path[-1])
...
def find_set(grid, point,code, Set=set()):
    for i in point.block_neighbours(grid):
        if i not in Set and not i.in_set :
            Set.add(i)
            i.in_set = code
...
def go_round(grid, start, end, current, path, line,code):
    Distance=dist(current,end)-dist(start,end)
    while True:
        if current in line and Distance<0:
            return path
        else:
            for i in current.plus_neighbours(grid):
                for j in i.block_neighbours(grid):
                    if j.in_set==code and i != path[-2]:
                        path.append(i)
                        current = i
                        return go_round(grid, start, end, current, path, line,code)
